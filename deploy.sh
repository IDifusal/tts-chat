...
echo "==> (3/9) Asegurando directorio data (SQLite persistente)..."
mkdir -p data

echo "==> (4/9) Bajando contenedores antiguos de tts-chat..."
docker compose -f "$COMPOSE_FILE" down 2>/dev/null || true

echo "==> (5/9) Limpieza ligera (imágenes dangling y build cache)..."
docker image prune -f >/dev/null 2>&1 || true
docker builder prune -f >/dev/null 2>&1 || true

echo "==> (6/9) Construyendo imagen actualizada (sin cache)..."
docker compose -f "$COMPOSE_FILE" build --no-cache

echo "==> (7/9) Levantando tts-chat (DB en ./data/streams.db, volumen montado)..."
docker compose -f "$COMPOSE_FILE" up -d

echo "==> (8/9) Esperando 8s..."
sleep 8
